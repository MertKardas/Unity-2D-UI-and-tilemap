...
public class PlayerController : MonoBehaviour, ISavable
{
    ...
    private void Start() {
        ...
        //Place the player if data exists
        if (!isNewgame) {
            transform.position = new Vector3(playerData.position[0], playerData.position[1], transform.position.z);
        } else { Debug.Log("Player data is null on start."); }
    }
